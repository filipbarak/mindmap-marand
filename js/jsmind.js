...
(function($w){
    ...
    var __name__ = 'jsMind';
    // library version
    var __version__ = '0.1a';
    // author
    var __author__ = 'filip@vertex.com.mk';
    ...
    jm.prototype={
        ...
        set_node_border:function(nodeid, border, borderrad, bordernone, margin){
            if(this.get_editable()){
                var node = this.mind.get_node(nodeid);
                if(!!node){
                    if(!!border){
                        node.data['border-bottom'] = border;
                    }
                    if(!!borderrad){
                        node.data['border-radius'] = borderrad;
                    }
                    this.view.reset_node_custom_style(node);
                    if(!!bordernone){
                        node.data['border'] = bordernone;
                    }
                    if(!!margin){
                        node.data['margin'] = margin;
                    }
                    this.view.reset_node_custom_style(node);
                }
            }else{
...
// ============= data provider =============================================
    ...
    jm.view_provider.prototype={
        ...
        _reset_node_custom_style:function(node_element, node_data){
            ...
            if('border-bottom' in node_data){
                node_element.style.borderBottom = node_data['border-bottom'];
            }
            if('border-radius' in node_data){
                node_element.style.borderRadius = node_data['border-radius'];
            }
            if('border' in node_data){
                node_element.style.border = node_data['border'];
            }
            if('margin' in node_data){
                node_element.style.margin = node_data['margin'];
            }
